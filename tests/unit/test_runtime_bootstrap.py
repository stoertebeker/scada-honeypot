...
def test_build_local_runtime_allows_open_meteo_without_egress_approvals(
    tmp_path: Path,
    monkeypatch,
) -> None:
    env_file = tmp_path / ".env"
    ...
    assert live_provider.provider_name == "open_meteo_forecast"
    assert history_provider.provider_name == "open_meteo_archive"
