...
- `APPROVED_EGRESS_TARGETS` covers every active exporter and the enabled GeoIP
  auxiliary target
- `APPROVED_EGRESS_CIDRS` covers every resolved exporter and GeoIP A/AAAA
  address and the same CIDRs are enforced by the host or perimeter firewall
...
- exporter or GeoIP traffic goes to an unapproved target
