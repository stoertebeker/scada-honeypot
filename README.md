...
- browserbasierte `Playwright`-Smokes unter `tests/e2e/test_hmi_service_playwright.py` pruefen jetzt `/service/login -> /service/panel -> breaker open -> /alarms`, `power_limit -> /overview -> /trends`, `breaker open -> breaker close -> /meter -> /alarms`, `inverter block control -> /inverters` sowie `block reset after COMM_LOSS_INVERTER_BLOCK -> /inverters -> /alarms` gegen den echten lokalen Runtime-Pfad
