...
- read-only HMI fuer `/`, `/overview`, `/single-line` und `/inverters` steht als `FastAPI`-/`Jinja2`-App auf derselben Snapshot-Wahrheit wie Modbus
...
- der HMI-Dienst laeuft als echter lokaler HTTP-Server; `GET /overview`, `GET /single-line` und `GET /inverters` sind damit nicht mehr nur im ASGI-Testpfad, sondern im Runtime-Slice erreichbar
...
- weitere HMI-Seiten jenseits von `overview`, `single-line` und `inverters`
...
1. naechste read-only HMI-Seite auf die jetzt laufende lokale HMI setzen, bevorzugt `weather`
