...
- `exporter_runner` mit leased Outbox-Drain, `WebhookExporter`, `SmtpExporter`, `TelegramExporter`, Retry-Backoff und einem lokalen Hintergrunddienst; Export-Ausfaelle landen kontrolliert in `outbox.retry_count`, `next_attempt_at` und `last_error`, ohne den Kernpfad zu blockieren, und Webhook, SMTP und Telegram liefern die rule-basierten Folge-Alerts `GRID_PATH_UNAVAILABLE`, `LOW_SITE_OUTPUT_UNEXPECTED` und `MULTI_BLOCK_UNAVAILABLE` jetzt bis zum Exporter-Double aus, auch parallel fuer denselben Folgealarm
- Release-Gate- und Hardening-Tests pruefen jetzt ruhige `401/403/404`-Seiten, fehlende `Server`-/`Date`-Header, lokale Bindings, Suppression gegen Folge-Alert-Flooding sowie Webhook-/SMTP-/Telegram-Ausfall ohne sichtbare Client-Seiteneffekte; Webhook-, SMTP- und Telegram-Hintergrundrunner decken dabei die rule-basierten Folge-Alerts `GRID_PATH_UNAVAILABLE`, `LOW_SITE_OUTPUT_UNEXPECTED` und `MULTI_BLOCK_UNAVAILABLE` kontrolliert ab, inklusive isoliertem Teilausfall eines Zielkanals bei laufender Mehrfachausleitung
...
- `uv run python -m honeypot.main --verify-exposed-research` fuehrt jetzt einen lokalen Start-/Read-/Alert-/Stop-Sweep fuer den freigegebenen Exponierungsmodus aus, prueft Modbus, HMI sowie den Alert-Pfad `BREAKER_OPEN` und schreibt dabei einen nachvollziehbaren Sweep-Eintrag nach `FINDINGS_LOG_PATH`
...
- Gesamtteststand aktuell: `273 passed`
...
1. erst danach weitere V1-Erweiterungen oder Exponierungskurse
...
explizit `ALLOW_NONLOCAL_BIND=1`, passende `APPROVED_INGRESS_BINDINGS` und
darf erst nach den Security-Gates erfolgen.
