...
- browserbasierte `Playwright`-Smokes unter `tests/e2e/test_hmi_service_playwright.py` pruefen jetzt `/service/login -> /service/panel -> breaker open -> /alarms`, `power_limit -> /overview -> /trends`, `breaker open -> breaker close -> /meter -> /alarms` sowie `inverter block control -> /inverters` gegen den echten lokalen Runtime-Pfad
