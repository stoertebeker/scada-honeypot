...
- `APPROVED_EGRESS_TARGETS` covers every active exporter and each enabled
  Open-Meteo/GeoIP auxiliary target
- `APPROVED_EGRESS_CIDRS` covers every resolved exporter, weather, and GeoIP
  A/AAAA address and the same CIDRs are enforced by the host or perimeter
  firewall
...
- exporter, weather, or GeoIP traffic goes to an unapproved target
