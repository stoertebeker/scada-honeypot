...
- read-only HMI fuer `/`, `/overview` und `/single-line` steht als `FastAPI`-/`Jinja2`-App auf derselben Snapshot-Wahrheit wie Modbus
...
- der HMI-Dienst laeuft als echter lokaler HTTP-Server; `GET /overview` und `GET /single-line` sind damit nicht mehr nur im ASGI-Testpfad, sondern im Runtime-Slice erreichbar
...
- weitere HMI-Seiten jenseits von `overview` und `single-line`
...
1. naechste read-only HMI-Seite auf die jetzt laufende lokale HMI setzen, bevorzugt `inverters`
